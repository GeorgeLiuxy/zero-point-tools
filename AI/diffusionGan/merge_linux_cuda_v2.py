...
class AddGaussianNoise:
    ...
    def __call__(self, tensor):
        """
        向输入张量添加高斯噪声。
        :param tensor: 输入的图像张量，形状为 [C, H, W]。
        :return: 加入高斯噪声后的图像张量。
        """
...
def save_comparison_image_with_enhancement(ir_image, visible_image, fake_image, epoch):
    """
    保存红外图像、可见光图像和融合图像的对比，并将增强后的融合图像也添加到拼接图像中。
    """
    # 转换张量为 PIL.Image
    ir_image = transforms.ToPILImage()(ir_image.cpu().squeeze(0))
    visible_image = transforms.ToPILImage()(visible_image.cpu().squeeze(0))
    ...
    enhance_and_colorize(fake_image, visible_image, enhanced_image_path)  # 生成增强图像
    enhanced_image = Image.open(enhanced_image_path)  # 加载增强后的图像
    enhanced_image = add_text_to_image(enhanced_image, "Enhanced Fused Image")

    # 拼接图像（红外图像 + 可见光图像 + 原始融合图像 + 增强后的融合图像）
    combined_width = ir_image.width * 4
    combined_height = ir_image.height
    combined_image = Image.new("RGB", (combined_width, combined_height))

    combined_image.paste(ir_image, (0, 0))
    combined_image.paste(visible_image, (ir_image.width, 0))
    combined_image.paste(fake_image, (ir_image.width * 2, 0))
    combined_image.paste(enhanced_image, (ir_image.width * 3, 0))  # 添加增强图像

    # 保存最终拼接图像
    comparison_path = f"output/comparison_with_enhanced_epoch_{epoch}.png"
    combined_image.save(comparison_path)
    print(f"Combined comparison image saved at: {comparison_path}")
...
if __name__ == '__main__':
    ...
    transform = transforms.Compose([
        ...
        transforms.ToTensor(),
        # AddGaussianNoise(mean=0.0, std=0.05),  # 自定义高斯噪声
    ...
    d_losses = []  # D Loss 的记录值
    g_losses = []  # G Loss 的记录值
    # **训练过程：调用伪彩色和细节增强**
    num_epochs = 100
    ...
    # 保存图像到文件
    plt.savefig("loss_curve.png")  # 将图像保存为 loss_curve.png
