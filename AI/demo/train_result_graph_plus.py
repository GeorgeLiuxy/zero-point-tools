...
CONFIG = {
    'num_agents': 50,               # 增加代理数
    'obs_dim': 32,                  # 扩展节点观测特征维度
    'hidden_dim': 64,               # 增大隐藏层维度
    'num_actions': 4,               # 扩展动作维度
    'target_radius': 0.1,
    'collision_radius': 0.2,
    'num_epochs': 200,              # 增加训练轮次
    'learning_rate': 1e-3,
    'batch_size': 8,                # 增大批量大小
    'edge_attr_dim': 8,             # 扩展边特征维度
    'history_len': 10               # 增加历史长度
}
...
class RLAgent(nn.Module):
    def __init__(self, input_dim, action_dim):
        ...
        self.fc = nn.Sequential(
            nn.Linear(input_dim, 128),  # 将64改为spatio_temporal_output的实际输出维度32
            nn.ReLU(),
...
class SpatioTemporalNet(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels):
        ...
        self.tcn = TemporalConvNet(num_inputs=hidden_channels * 4, num_channels=[hidden_channels, 64, 32])
...
def main():
    ...
    # 生成初始数据
    data_list = [generate_data_for_agent(i, num_nodes=num_agents, obs_dim=obs_dim, edge_attr_dim=edge_attr_dim, history_len=history_len) for i in range(10 * num_agents)]
    ...
    agent = RLAgent(input_dim=32, action_dim=num_actions)  # 将input_dim设为32
...
# 数据生成函数
def generate_data_for_agent(agent_id, num_nodes=10, obs_dim=32, edge_attr_dim=8, history_len=10):
