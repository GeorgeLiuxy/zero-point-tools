...
# 路径可视化函数
def plot_paths(paths, target_pos, obstacles=None, observation_radius=0.5):
    plt.figure(figsize=(10, 8))
    paths = torch.stack(paths).numpy()  # 将路径堆叠成 numpy 数组，方便绘图
    num_steps = paths.shape[0]  # 总的步骤数，用于颜色渐变
    ...
    for agent_idx in range(paths.shape[1]):
        # 绘制观测范围
        for point in paths[:, agent_idx]:
            circle = plt.Circle((point[0], point[1]), observation_radius, color='gray', alpha=0.1)
            plt.gca().add_patch(circle)

        # 按顺序绘制路径中的点，用颜色表示先后顺序
        for step in range(num_steps - 1):
            color = cmap(step / num_steps)  # 计算渐变颜色
            plt.plot(
                [paths[step, agent_idx, 0], paths[step + 1, agent_idx, 0]],
                [paths[step, agent_idx, 1], paths[step + 1, agent_idx, 1]],
                color=color,  # 使用渐变色
                linewidth=2
            )
            # 在每个点绘制标记，可以观察顺序
            plt.scatter(paths[step, agent_idx, 0], paths[step, agent_idx, 1], color=color, s=20)

            # 在路径的最后一个点和目标点之间绘制连接线
            final_position = paths[-1, agent_idx]  # 获取路径的最后一个位置
            plt.plot(
                [final_position[0], target_pos[agent_idx, 0].item()],
                [final_position[1], target_pos[agent_idx, 1].item()],
                'k--',  # 黑色虚线
                linewidth=1.5
            )
            # 绘制从当前点到目标点的虚线
            # plt.plot(
            #     [paths[step, agent_idx, 0], target_pos[agent_idx, 0].item()],
            #     [paths[step, agent_idx, 1], target_pos[agent_idx, 1].item()],
            #     'k--',  # 黑色虚线
            #     linewidth=0.5,
            #     alpha=0.3  # 设置透明度，避免图形过于拥挤
            # )

        # 绘制智能体的目标点
        plt.scatter(target_pos[agent_idx, 0].item(), target_pos[agent_idx, 1].item(), color='red', marker='x', label=f'Target {agent_idx + 1}' if agent_idx == 0 else None)

    plt.xlabel("X Position")
    plt.ylabel("Y Position")
    plt.legend()
    plt.title("Planned Paths with Obstacles and Sequential Target Lines")
    plt.show()
...
def main():
    ...
    for data, obstacles in data_list:
        ...
        plot_paths(sca_path, data.y, obstacles=obstacles)
        sca_steps, sca_length, sca_final_distance = calculate_path_metrics(sca_path, data.y)

        # 使用RL模型的路径规划
        with torch.no_grad():
            rl_output = network(data)
            rl_actions = agent(rl_output)
            rl_steps, rl_length, rl_final_distance = calculate_path_metrics(rl_actions, data.y)
    ...
    #     plot_paths(paths, data.y, obstacles=obstacles)
    # for data, obstacles in data_list:
    #     paths = sca_path_planning(data)
    #     # logger.info(f"Planned paths for agent: {paths}")
    #     plot_paths(paths, data.y, obstacles=obstacles)
