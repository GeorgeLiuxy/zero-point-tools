...
# 提供年份和月份，爬取对应的的表格数据
url = "http://tianqi.2345.com/wea_history/54161.htm"
headers = {
...
def craw_table(id,year,month):
    ...
    print("Response Text (first 200 chars):", resq.text[:200])
    if resq.status_code != 200:
        raise Exception("Request failed with status", resq.status_code)
    try:
        data = resq.json()["data"]
    ...
    print("Response status:", resq.status_code)
    print("Response text (snippet):", resq.text[:200])
    return df
