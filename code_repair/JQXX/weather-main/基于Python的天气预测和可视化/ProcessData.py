...
def setAir(df):
    # 用气压的倒数乘以风速做简单近似（仅为演示，非真实 AQI 模型）
    return [round((1 / p) * w * 1000, 2) if p and w else 0 for p, w in zip(df["pres"], df["wspd"])]
...
#     return temperature
def setHighTemp(df):
    return [round(x, 2) for x in df["最高温"] if pd.notnull(x)]
...
def setLowTemp(df):
    return [round(val, 1) for val in df['最低温'] if pd.notnull(val)]
...
def process(date):
    ...
    # 安全删除可能不存在的列
    if '天气' in date.columns:
        date = date.drop('天气', axis=1)
    if '风力风向' in date.columns:
        date = date.drop('风力风向', axis=1)
...
def write(years, b,c):
    ...
    id =54161
    # 取到预处理后的用来预测的数据
    date0 = weather_utils.getPredictDate(week_ago.year - years[0], week_ago.month, week_ago.day, week_pre.year - years[1], week_pre.month, week_pre.day)
